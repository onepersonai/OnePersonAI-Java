...
import com.opai.auth.KeyCloakTokenService;
import com.opai.blob.FileSystemService;
import com.opai.blob.IBlobService;
import com.opai.blob.S3BlobService;
import com.opai.rest.*;
import com.opai.util.*;
import com.opai.auth.OPAITokenService;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import com.opai.acl.AppAcl;
import com.opai.acl.InvitationCommentACL;
import com.opai.bus.MailHandler;


import com.opai.bus.VMHeater;
import com.opai.model.Annotation;
...
public class MATC extends AbstractVerticle {
	...
	public void start() {
		...
		System.out.println("******************************************");
		System.out.println("* OnePersonAI-Server " + VERSION + " launched at " + config.getInteger("http.port") + "");
		System.out.println("******************************************");
	}
	...
	public void initTokenService(JsonObject config) {
		if (Config.isKeyCloak(config)) {
			...
			initOPAITokenService(config);
		}
	}

	private void initOPAITokenService(JsonObject config) {
		OPAITokenService tokenService = new OPAITokenService();
		if (config.containsKey(Config.JWT_PASSWORD)){
			...
			if (jwtSecret.trim().length() > 0) {
				...
				logger.error("initOPAITokenService() > Password is empty");
				tokenService.setSecret(Util.getRandomString());
			}
		} else {
			tokenService.setSecret(Util.getRandomString());
			logger.error("initOPAITokenService() > No key. Use random!");
		}
	...
	public void stop(){
		...
		try {
			...
			System.out.println("******************************");
			System.out.println("* OnePersonAI Server STOP       *");
			System.out.println("******************************");
